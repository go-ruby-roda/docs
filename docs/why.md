...
`go-ruby-roda/roda` reimplements Ruby's `roda` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-roda/roda` directly, with no Ruby runtime;
...
Because `roda` is pure Go with cgo disabled, it also compiles to **WebAssembly**
